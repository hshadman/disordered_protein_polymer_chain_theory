      program RWchains
c     Modifying to work for Random Walk chains.
c     I think what I am going to do here is have empty keep track of the
c        number of chain beads are on that site.
c     If empty = 0, the site is empty.
c     If empty = 1, there is one chain bead there.
c     If empty = 2, there are two different chain beads. . . .
c     Then, I can just subtract 1 from empty when the chain moves.
c
c     For all of the changes, I will start with a comment line that has:
c     'RW modified'
c
c     I may need to modify endfind, ebfindnew, and bfind subroutines,
c        but I am leaving that alone for now. My initial thought is that
c        they do not need to be modified because you do not pass 
c        empty to those subroutines.
c
c    ===JZ   Sept 13 2022
c
c     I am going to try to get the reptation move working first, then worry 
c       about onebead and twobead moves so I set frep = 1.2
...
c     modified to set a bulk lattice
      ...
      do 3072 i=1,nch
         do 3073 j=1,lch
c     RW modified
            empty(chains(i,j,1),chains(i,j,2),chains(i,j,3))=
...
c     RW modified
      write(13,*) 'program is RWchain version of Sept 2022'
...
c     RW modified
      integer newchains(nchmax,lchmax,3),ipk
...
c     first decide move head or ends
      ...
      do k=1,3
         nb(k)=chain(1,k)
c     RW modified
c         nbminus(k)=chain(2,k)
         ntail(k)=chain(lch,k)
      end do


c     RW modified
c     get a new position to move the head bead to
      call n4mod(nb,nn)
      call randu(6.0,ipk)
      do k = 1,3
         enpnew(k) = nn(ipk,k)
...
c     RW modified
c     make a copy of chains
      do i=1,nch
         do j=1,lch
            do k=1,3
               newchains(i,j,k)=chains(i,j,k)
            end do
         end do
      enddo
c     now move the chain
      do k=1,3
...
c     need to update newchains
      do j=1,lch
         do k=1,3
            newchains(ichain,j,k)=chain(j,k)
         end do
      end do
c     now update empty
      empty(ntail(1),ntail(2),ntail(3)) =
     $     empty(ntail(1),ntail(2),ntail(3)) - 1
      empty(enpnew(1),enpnew(2),enpnew(3)) =
     $     empty(enpnew(1),enpnew(2),enpnew(3)) + 1
c     calculate the number of contacts for the new position
      call CALNAB(NASNEW,empty,newchains,lsxmax,lsymax,lszmax,
     $     nchmax,lchmax)
...
c     RW modified
         ...
         do j=1,lch
            do k=1,3
               chains(ichain,j,k)=newchains(ichain,j,k)
            end do
         enddo
      else
c     if move is not successful, you need to fix empty
         empty(ntail(1),ntail(2),ntail(3)) =
     $        empty(ntail(1),ntail(2),ntail(3)) + 1
         empty(enpnew(1),enpnew(2),enpnew(3)) =
     $        empty(enpnew(1),enpnew(2),enpnew(3)) - 1
...
c     *******************************************************************
     ...
     $ lsxmax,lsymax,lszmax)
      ...
      integer chainscont(nchmax,lchmax,3)
c     RW modified
      integer newchains(nchmax,lchmax,3)
c      double precision vpotential(lsxmax,lsymax,lszmax),deltaV,ex
...
c     now move
         do k=1,3
            newchains(ichain,jbead,k)=enpnew(k)
         end do
c     RW modified-- chainscont is not updated during reptation
c       so I am not going to update it here
c     now update chainscont
c         
c         do k=1,3
c            dx(k)=enpnew(k)-enpold(k)
c            if(dx(k).eq.(1-lsl(k))) dx(k)=+1
c            if(dx(k).eq.(lsl(k)-1)) dx(k)=-1
c            chainscont(ichain,jbead,k)=chainscont(ichain,jbead,k)
c     $           +dx(k)
c     end do
         empty(nb(1),nb(2),nb(3))=
     $        empty(nb(1),nb(2),nb(3)) - 1
         empty(enpnew(1),enpnew(2),enpnew(3))=
     $        empty(enpnew(1),enpnew(2),enpnew(3)) + 1
...
c      end do
      ...
      isite=0
      do j=1,6
         isitenew=lsx*lsy*nn(j,3)+lsx*nn(j,2)+nn(j,1)
         if(isitenew.ne.isiteold2) then
            isite=isite+1
            do k=1,3
               mm(isite,k)=nn(j,k)
            end do
      ...
      if(isite.eq.6) then
         write(*,*) 'isite',isite,'isiteold2',isiteold2,
     $ 'isiteold',isiteold
         do k=1,3
            write(*,*) 'enpold',enpold(1),enpold(2),enpold(3)
            write(*,*) 'enpold2',enpold2(1),enpold2(2),enpold2(3)
         end do
         do j=1,6
            isitenew=lsx*lsy*nn(j,3)+lsx*nn(j,2)+nn(j,1)
            write(*,*) 'isitenew',isitenew
         end do
      end if
c     now mm contained 5 sites, the enpold2 is eliminated
...
c     RW modified
c     do the move
c     I took out the part about updating chainscont
      do i=1,nch
         do j=1,lch
            do k=1,3
               newchains(i,j,k)=chains(i,j,k)
      ...
      if(nmove.eq.1) then
         do k=1,3
            newchains(ichain,jbead,k)=enpnew1(k)
            newchains(ichain,jbead+1,k)=enpnew2(k)
         end do
      else if(nmove.eq.-1) then
         do k=1,3
            newchains(ichain,jbead,k)=enpnew1(k)
      ...
      empty(enpold1(1),enpold1(2),enpold1(3))=
     $     empty(enpold1(1),enpold1(2),enpold1(3)) - 1
      empty(enpold2(1),enpold2(2),enpold2(3))=
     $     empty(enpold2(1),enpold2(2),enpold2(3)) - 1
      empty(enpnew1(1),enpnew1(2),enpnew1(3))=
     $     empty(enpnew1(1),enpnew1(2),enpnew1(3)) + 1
      empty(enpnew2(1),enpnew2(2),enpnew2(3))=
...
c     RW modified
c     Adding an extra check to get the sum of empty
       empty_sum = 0
       do i=1,lsx
          do j=1,lsy
             do k=1,lsz
                empty_sum = empty_sum + empty(i,j,k)
             enddo
          enddo
       enddo
       write(12,*) 'empty_sum ',empty_sum
       if(empty_sum.ne.lch*nch) then
          IFINE=0
          write(12,*) 'EMPTY and lch*nch do not match'
       endif
...
c     RW modified
             if(empty(nb(1),nb(2),nb(3)).lt.1) then
                ifine=0
